...
import { AlertCircle, Plus, X, Loader2, ChevronRight, Wrench, Zap, Droplets, PaintBucket, Trash2, Image as ImageIcon } from 'lucide-react';
import { getMaintenanceRequests, createMaintenanceRequest, updateRequestStatus, uploadTenantPhoto } from '@/app/actions';
...
export const RequestsTab = ({ managerId, properties }: { managerId: string; properties: any[] }) => {
  ...
  const fetchRequests = async (p: number) => {
    ...
    const res = await getMaintenanceRequests(managerId, p, filters);
    if (res.success) {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < Math.min(files.length, 4 - newPhotos.length); i++) {
      ...
      const res = await uploadTenantPhoto(fd);
